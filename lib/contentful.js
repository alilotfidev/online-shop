...
export async function getProduct(sku) {
  const res = await fetch(
    createUrl(`/entries`, ['content_type=product', `fields.sku[match]=${sku}`])
  );
...
export async function getUserOrders(userId) {
  const res = await fetch(
    createUrl(`/entries`, [
      'content_type=order',
      `fields.customer.sys.id[match]=${userId}`,
    ])
  );
...
export async function addProductsToOrder(entryId, object, totalPrice) {
  const client = await initializeClient();
  ...
  entry.fields.totalAmount = { 'en-US': totalPrice };

  // Save the updated entry
  await entry.update();
  await publishEntry(entry.sys.id);
}
export async function setOrderPaymentStatus(entryId, paid) {
  const client = await initializeClient();

  const entry = await client.getEntry(entryId);
  if (paid) {
    entry.fields.status = { 'en-US': 'checking' };
    entry.fields.isPaid = { 'en-US': true };
  } else {
    entry.fields.status = { 'en-US': 'canceled' };
    entry.fields.isPaid = { 'en-US': false };
  }
  // Save the updated entry
  await entry.update();
  await publishEntry(entry.sys.id);
